...
\end{itemize}
\paragraph{Funzionamento}
Memorizza dapprima un mongolog per tenere traccia dell'operazione. Entrando nel \texttt{try} poi se \texttt{dst} termina con
una slash allora non è stato dato il nome del nuovo file e quindi questo sarà lo stesso di quello originale. A tale scopo a
\texttt{dst} viene aggiunto il risultato della funzione \texttt{path.basename(src)} che è proprio il nome del file originale.\\
...
\begin{itemize}
	...
	\item{se invece \texttt{onlydependences=False} ci si trova in una condizione in cui l'output può
		...
		Questa funzionalità è usata ad esempio dalla funzione \nameref{aptinstall} durante la
