...
Si deduce che gli oggetti che è possibile ottenere sono i \textit{virtual hosts} ossia i siti che apache gestisce,
...
cartella \textit{enabled} dell'oggetto voluto (ad esempio \textit{sites-enabled}). Si aggiunge quindi ognuno di questi oggetti al dizionario
\texttt{objs} assegnandosi anche il flag \texttt{active} col valore 1, ad indicare che l'oggetto è attivo.\\
...
che non sono attivi, e li si inserisce nel dizionario \texttt{objs} con la flag \texttt{active} a 0 ad indicare che tali oggetti
non sono attivi.
\paragraph{Return}
