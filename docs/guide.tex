...
\includegraphics[width=\textwidth]{param.png}\\

\subsubsection{param}\label{param}
\begin{lstlisting}
@app.route('/param')
def param():
	error = None
...
\end{lstlisting}
Il servizio richiama il metodo hostname e restituisce hostname della macchina che è possibile modificare.
Inoltre raccoglie in due liste alcuni file e cartelle su cui è possibile visualizzarne il contenuto.\\

\subsubsection{newHostname}\label{newHostname}
\begin{lstlisting}
...
def newHostname():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
Il servizio che recupera il nuovo hostname scelto dall'utente tramit un form in pagina e lo modifica.

\subsubsection{retriveFileDir}\label{retriveFileDir}
\begin{lstlisting}
...
def retriveFileDir():
	...
	return redirect(url_for('param'))
\end{lstlisting}
Il servizio recupera il path dalla lista delle cartella e ne legge il contenuto.

\subsection{users.html}\label{users.html}

\includegraphics[width=\textwidth]{usergroups.png}\\

\subsubsection{listUserAndGroups}\label{listUserAndGroups}
\begin{lstlisting}
...
\end{lstlisting}
Il servizio richiama tre metodi getusers() e  getgroups() per mostrare il risultato in due tabelle e getshells() servito per il form add user, in modo da popolare la dropdown per la selezione della shell.

\subsubsection{getInfoUser}\label{getInfoUser}
\begin{lstlisting}
...
\end{lstlisting}
IL servizio accetta in input un parametro che corrisponde all'uname. Il parametro viene passato in input alla funzione getuser() e restituisce le informazioni del singolo uname. Infatti, in pagina, se si clicca sul singolo uname, si viene riportati in una pagina con le informazioni dell'uname.
Inoltre, nello stesso servizio vengono richimati altri 4 metodi, getshells() servito per il form per la modifica della shell, in modo da popolare la dropdown per la selezione della shell.
Gli altri metodi sono getusernotgroups() e getusergroups() che sono serviti in pagina per le form di aggiunta user ai gruppi e rimozione user dai gruppi.

\subsubsection{updateShell}\label{updateShell}
\begin{lstlisting}
...
def updateShell():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
Il servizio recupera l'uname e la shell da modifica e richiama updateusershell(), a meno che l'user non selezioni un'opzione non valida.

\subsubsection{addUserGroup}\label{addUserGroup}
\begin{lstlisting}
...
def addUserGroup():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
Il servzio aggiunge l'uname recuperato a una lista di gruppi che l'utente seleziona in pagina. La funzione della request utilizzata è getlist(). I parametri recuperati vengono passati alla funzione addusertogroups(), a meno che uname non sia vuoto o tra i gruppi selezionati non ci sia un'opzione non valida.

\subsubsection{removeUserGroup}\label{removeUserGroup}
\begin{lstlisting}
...
def removeUserGroup():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
Il servzio rimuove l'uname recuperato a una lista di gruppi che l'utente seleziona in pagina. La funzione della request utilizzata è getlist(). I parametri recuperati vengono passati alla funzione removeuserfromgroups(), a meno che uname non sia vuoto o tra i gruppi selezionati non ci sia un'opzione non valida.

\subsubsection{updateUserPwd}\label{updateUserPwd}
\begin{lstlisting}
...
def updateUserPwd():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
Il servizio recupera uname e la nuova password, per l'aggiornamento della password utente e richiama il metodo updateuserpass(), a meno che il campo passord non sia vuoto.
...
def addUser():
	...
	return redirect(url_for('listUserAndGroups'))
\end{lstlisting}
Il servizio tramite una form, recupera user, password e shell per l'aggiunta di un nuovo utente e richiama il metodo adduser(), a meno che il campo user e passord non siano vuoti.\\
...
def removeUser():
	...
	return redirect(url_for('listUserAndGroups'))
\end{lstlisting}
Il servizio tramite una form, recupera user che si desidera eliminare, infatti richiama il metodo removeUser(), a meno che il campo user non sia vuoto.

\subsection{jobs.html}\label{jobs.html}

\includegraphics[width=\textwidth]{cron1.png}\\
\includegraphics[width=\textwidth]{cron2.png}\\

\subsubsection{listCron}\label{listCron}
\begin{lstlisting}
@app.route('/listCron')
def listCron():
...
\end{lstlisting}
Il servizio richiama il metodo listcrontabs() che restituisce la lista dei cron la lista dei cron che sono stati organizzati per chiave in pagina.

\subsubsection{getContentCrontab}\label{getContentCrontab}
\begin{lstlisting}
...
\end{lstlisting}
Il servizio costruisce il path iniziale del cron, concatenando il path restante e il nome del cron che vengono passati in input. Eseguo il metodo readfile() che restituisce il contenuto del cron.

\subsubsection{updateCrontab}\label{updateCrontab}
\begin{lstlisting}
...
def updateCrontab():
	try:
		...
		return internal_server_error(500)	
\end{lstlisting}
Una volta in pagina, se si esegue una modifica, il bottone di modifica si abilita e viene richiamato il metodo writefile().

\subsubsection{deleteCron}\label{deleteCron}
\begin{lstlisting}
...
def deleteCron():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
Il servizio elimina il cron selezionato dalla dropdown e richiama il metodo filedel().
...
def addCron():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
Viene compilata una form in pagina, l'unico campo obbligatorio è il campo Comando. Il servizio recupera tutti i parametri selezionati dall'utente e richiama addcron().

\subsubsection{addCustomCron}\label{addCustomCron}
\begin{lstlisting}
...
def addCustomCron():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
Viene compilata una form in pagina, l'unico campo obbligatorio è il campo Comando, mentre in base al tipo selezionato dall'utente, viene richiamato un metodo diverso: se l'utente sceglie ogni ora, verrà richiamato addhourlycron(), se sceglie ogni giorno, verrà richiamato adddailycron(), ecc..

\subsection{file.html}\label{file.html}
...
\end{lstlisting}
Mostra univocaente la pagina senza alcuna logica particolare.

\subsubsection{findFile}\label{findFile}
\begin{lstlisting}
...
\end{lstlisting}
Il servizio viene richiamato quando l'utente ricerca un file all'interno del filesystem. Richiama il metodo locate() e gli passa in input la stringa ricercata dall'utente.

\subsubsection{updateDbFile}\label{updateDbFile}
\begin{lstlisting}
...
def updateDbFile():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
Il servizio richiama updatedb(). In pagina viene passato un valore hidden che permette di identificare il bottone che lancia il servizio.

\subsubsection{retriveContentFile}\label{retriveContentFile}
\begin{lstlisting}
...
def retriveContentFile():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
Il servizio richiama readfile() per recuperare il contenuto di un file. Viene richiamato più volte all'interno all'interno dell'applicativo.

\subsubsection{updateFile}\label{updateFile}
\begin{lstlisting}
...
def updateFile():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
Il servizio serve a rendere effettive le modifiche fatte al contenuto di un file. Richiama il metodo writefile().

\subsubsection{deleteFile}\label{deleteFile}
\begin{lstlisting}
...
def deleteFile():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
IL servizio serve ad eliminare un file. Recupera il path del file e lo passa alla funzione filedel(). Per vedere le modifiche dall'applicativo, senza obbligare l'utente a rieseguire l'update del db dei file, la funzionalità di viene richiamata nuovamente all'interno del servizio.

\subsubsection{copyFile}\label{copyFile}
\begin{lstlisting}
...
def copyFile():
	...
	if request.form['copyFile'] == 'Copia':
		...
		error = 'Non funzica'	
\end{lstlisting}
Il servizio viene richiamato per effettuare la copia di un file, richiamando la funzione filecopy(). Da una form, è possibile selezionare il path di destinazione, dove si vuole salvare la copia desiderata.Come per l'eliminazione dei file, per vedere le modifiche dall'applicativo, senza obbligare l'utente a rieseguire l'update del db dei file, la funzionalità di viene richiamata nuovamente all'interno del servizio.

\subsubsection{renameFile}\label{renameFile}
\begin{lstlisting}
...
def renameFile():
	...
	if request.form['renameFile'] == 'Rinomina':
		...
		return render_template("file.html",error=error)
\end{lstlisting}
Il servizio viene richiamato per rinominare un file, richiamando la funzione filerename(). Da una form, è possibile selezionare il nome nuovo del nostro file.Come per l'eliminazione e la copia dei file, per vedere le modifiche dall'applicativo, senza obbligare l'utente a rieseguire l'update del db dei file, la funzionalità di viene richiamata nuovamente all'interno del servizio.

\subsection{applications.html}\label{applications.html}

\includegraphics[width=\textwidth]{apps.png}\\
...
\end{lstlisting}
Restituisce la lista della app installate.

\subsubsection{findPkg}\label{findPkg}
\begin{lstlisting}
...
\end{lstlisting}
Avvia la ricerca dei paccheti installati e non installati e ne restiuisce la lista.
La ricerca viene filtrata in base ad una checkbox che setta namesonly a true (ricerca solo per nome) se ceccata.

\subsubsection{getInfoApp}\label{getInfoApp}
\begin{lstlisting}
...
\end{lstlisting}
Restituisce le info del pacchetto installato. Accetta in input il pacchetto selezionato dall'utente presente nella lista in pagina. REstituisce una lista di stringhe, quindi è stata usata la replac per sostituire gli accapo con quelli leggibili da html.

\subsubsection{removePackage}\label{removePackage}
\begin{lstlisting}
...
\end{lstlisting}
FUnzione utilizzata per la rimozione dei pacchetit installati.

\subsubsection{installPackage}\label{installPackage}
\begin{lstlisting}
...
def installPackage(name):
	...
	return render_template('applications.html',error=error)
\end{lstlisting}
FUnzione utilizzata per l'installazione dei pacchetti non presenti.

\subsubsection{retrieveExternalRepo}\label{retrieveExternalRepo}
\begin{lstlisting}
...
\end{lstlisting}
Resituisce i repository esterni. Inoltre viene richiamata getreponame() per generare un nome dinamico per le dropdown presenti nelle form in pagina.
...
def addRepo():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
Funzione per l'aggiunta di una nuova repo. Da una form, si popolano i dati come utente desidera. Il nome è autogenerato o modificabile dall'utente.

\subsubsection{getContentRepo}\label{getContentRepo}
\begin{lstlisting}
...
def getContentRepo():
	...
	return redirect(url_for('retrieveExternalRepo'))
\end{lstlisting}
Funzione che restituisce il contenuto di una repo, a meno di scelta invalidante.

\subsubsection{updateRepoFile}\label{updateRepoFile}
\begin{lstlisting}
...
def updateRepoFile():
	...
	#	return internal_server_error(500)
\end{lstlisting}
Funzione di update di un file repo, abilitata dopo che l'utente modifica il contenuto del repo presedentemente mostrato.

\subsubsection{removeRepo}\label{removeRepo}
\begin{lstlisting}
...
def removeRepo():
	...
	#	return internal_server_error(500)
\end{lstlisting}
Funzione di rimozione repo. Se il nome file contiene .list o .list.save, vengono "eliminati", affinchè la funzione removerepofile funzioni correttamente. 

\subsubsection{aggiornaCachePacchetti}\label{aggiornaCachePacchetti}
\begin{lstlisting}
...
def aggiornaCachePacchetti():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
FUnzione per l'aggiornamento dalla cache dei pacchetti. In pagina, è solo un bottone e nella funzione viene richiamata aptupdate(). 

\subsection{apache-sites.html-apache-modules.html-apache-configurations.html}\label{apache-sites.html-apache-modules.html-apache-configurations.html}

\includegraphics[width=\textwidth]{apache-sites.png}\\

\includegraphics[width=\textwidth]{apache-modules.png}\\

\includegraphics[width=\textwidth]{apache-configurations.png}\\
...
def statusApache():
	try:
		...
		return internal_server_error(500)
\end{lstlisting}
Sites, modules, configurations utilizzano le 5 funzioni in alto che servono per la gestione di apache server.\\
E' possibile agire su di sites, modules e configurations, attivandoli e disattivandoli (bottone rosso-bottono verde). Una volta effettuata una mdoifica in pagina, automaticamente tutti i bottoni tranne restart e reload sono disabilitati. Infatti, per verificare gli effetti di queste attivazioni/disattivazioni, è necessario agire prima sul apache.\\Successivamente saranno visibili i cambiamenti in pagina. 
