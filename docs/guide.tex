...
\subsection{Cron}
Il cron è l'applicazione inclusa di base in tutte le distribuzioni Linux based che copre il compito di eseguire
...
Il cron può essere di sistema o specifico di un utente; abbiamo deciso in questa libreria di lavorare solo sui
cron dell'utente in quanto i cron di sistema possono essere modificati o aggiunti creando o modificando i file
dalla sezione \nameref{file}. Non sono tuttavia da sottovalutare le possibilità di questo approccio in quanto
è possibile modificare anche il cron dell'utente \texttt{root} consentendo quindi l'esecuzione di qualsiasi
operazione.
\subsubsection{getusercron}
Restituisce al chiamante il contenuto del file di cron dell'utente.
\begin{lstlisting}
...
\end{lstlisting}
\paragraph{Parametri}
Prende un solo parametro che è il nome dell'utente di cui si vuole vedere il crontab.
\paragraph{Funzionamento}
Il crontab dell'utente viene creato la prima volta che dal terminale 
\paragraph{Return}
